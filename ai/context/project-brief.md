...
- 2026-05-28：Phase 2.3.18 browser-safe scheduler 架构补强已启动：继续使用浏览器 + TypeScript/Vite，不新增 runtime 第三方库；`requestAnimationFrame` 不再无界补 tick，浏览器层按固定 tick 数和毫秒预算消费仿真工作；Scheduler UI 增加 runtime/backlog、lane 计数和 diffusion counter；core 侧新增 DOM-free `TickPlan` / `TickReport` / scheduler lane stats，并把 #71 pressure diffusion profiling counters 纳入 scheduler 观测，包括 near-zero candidate 与实际 skipped 分离。Phase 2.3.19 已开始 pressure lane tightening：大世界 pressure diffusion 结合直接压力写入 chunk 与固定 deterministic background chunk slice，region summary refresh 计数改为跟随实际变化的 diffusion regions。Phase 2.3.22 已开始 agent/field lane split：agent-only 活跃 chunk 不再强制 immediate field scan；后续仍需 terrain/pressure profile 验证收益和生态漂移。
