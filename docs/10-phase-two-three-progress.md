...
- Phase 2.3.32 starts deterministic same-tick stack control between warm/sleeping catch-up and pressure diffusion. Large worlds now estimate catch-up cells before field updates and reduce same-tick pressure diffusion source/chunk budget by half on catch-up ticks; small worlds keep configured budgets. This does not depend on browser frame timing and does not disable diffusion, but it prevents catch-up and diffusion from both peaking at full width in the same core tick. New scheduler/profile counters expose `estimatedCatchUpUpdatedCells`, `effectivePressureDiffusionSourceBudget`, `effectivePressureDiffusionChunkBudget`, and `pressureDiffusionBudgetStaggered`. Validation: targeted core/profiler tests passed with `77` tests, `npm run check` passed with `101` tests, `npm run build` passed, and `npm run bench:core` completed with hot `step(16) ~358.90ms`. A visible 30s terrain-only browser profile at `http://127.0.0.1:5178/?profile=terrain&profileDetail=deep&profileSeconds=30` passed: `sim.step p50/p95 ~17.0ms/31.6ms`, `runtime.backlogTicks p95 ~1.0`, `render.total p95 ~2.4ms`, `core.world.diffusePressure p95 ~3.4ms`, `diffusionComputedCells p95 52224`, `effectivePressureDiffusionChunkBudget p95 64`, and `catchUpAndDiffusionCells p95 95232`.
...
- Phase 2.3.31 confirms the next structural risk is not overlay rendering but same-tick core work stacking. Phase 2.3.32 adds the first deterministic budget stagger for that stack; the remaining risk is pressure-visible correctness/performance and whether the reduced same-tick diffusion width creates unacceptable frontier lag in long browser profiles.
